...
WSL에서 서버를 실행한다면 Codex 기기 로그인은 **서버가 실행되는 WSL 터미널**에서
`codex login --device-auth`로 진행합니다. 로그인 상태가 확인돼도 모델이 표시되지 않으면
재로그인 대신 [설치와 인증 안내](docs/INSTALL.md)의 모델 목록 경로를 확인하세요.
